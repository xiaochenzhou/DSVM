...
def k_means_random_select (global_num, Edge_data, Edge_label, Edge_support_vector):
    ...
    for i in Edge_data:
        ...
        index += 1
    # print (return_label)
    K_plus = KMeans(n_clusters = k_node_num, random_state = 0)
    print ("plus size is %s"  %( np.size (Edge_data_plus, axis= 0 )))
    K_plus.fit (Edge_data_plus)
    K_minus = KMeans(n_clusters = k_node_num, random_state = 0)
    print ("minus size is %s "  %( np.size (Edge_data_minus,axis=0) ))
