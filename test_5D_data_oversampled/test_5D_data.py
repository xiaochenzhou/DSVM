...
# Train_data = np.load("Train_data_5D2.npy")
# Train_label = np.load("Train_label_5D2.npy")
...
Train_data,Train_label = shuffle(Train_data_t, Train_label_t)
...
# this part is responsible for random select
    ...
    additional_data, additional_label = random_select(Edge_data[i], Edge_label[i], Collect_support_vector_, 100)
...
whether_first_fit_flag = False
for i in Collect_label:
    if (i != temp):
        whether_first_fit_flag = True
        break

if (np.size(Collect_label) == 0):
    whether_first_fit_flag = False

if (whether_first_fit_flag):
    test.fit(Collect_support_vector_, Collect_label)
print ("begin")
print(np.size(Collect_label, axis=0))
...
while( (not SV_compare(old_support_vectors_, new_support_vectors_)) | (not whether_first_fit_flag)  ):
    print ("ite_count = %s" %(ite_count))
    whether_first_fit_flag = True
